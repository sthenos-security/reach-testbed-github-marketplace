...
func DiagnosticPing(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("DiagnosticPing ping failed for host %q: %v; output=%s", host, err, string(out))
		http.Error(w, "diagnostic check failed", http.StatusBadGateway)
		return
	}
...
func SafeDiagnosticPing(w http.ResponseWriter, r *http.Request) {
	...
	out, err := exec.Command("ping", "-c", "1", host).CombinedOutput()
	if err != nil {
		log.Printf("SafeDiagnosticPing ping failed for host %q: %v; output=%s", host, err, string(out))
		http.Error(w, "diagnostic check failed", http.StatusBadGateway)
		return
	}
